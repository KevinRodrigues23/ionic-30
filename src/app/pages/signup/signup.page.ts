...
export class SignupPage {
  ...
  createAccount() {
    this.usersService
      .createAccount(this.model.email, this.model.password)
      .then((result: any) => {
        this.toast.create({
          message: 'O Usuário foi criado com sucesso Token: ' + result.token,
          position: 'bottom',
          duration: 3000,
        }); //.present();
      ...
      .catch((error: any) => {
        this.toast.create({
          message: 'Erro ao criar usuário. Erro:  ' + error.error,
          position: 'bottom',
          duration: 3000,
        }); //.present();
      });
